    ...
    def __init__(self, instructions: JsonBB) -> None:
        ...
        for inst in instructions:
            ...
            if isinstance(inst, str):
                self.instr.append((CALL_INSTR_STUB, inst))
            else:
...
class CGNode:
    ...
    def find_similar(self, nodes: Iterable[CGNode], renames: RenameMapping=None) -> Optional[int]:
...
class CallGraph:
    ...
    """
    __slots__ = 'project', 'funcs', 'max_level', 'levels'

    def __init__(self, project_name: str, version_index: int = None) -> None:
        """ Constructor
        ...
        filename = f'{CG_PREFIX}_{project_name}'
        if version_index is not None:
            # We are loading a CG from some previous program version
            filename = f'{OLD_CG_PREFIX}_{project_name}_{version_index}'
        # Construct the node mapping from a JSON file
        self.funcs: NodeMap = self._parse_json_cg(load.load_and_deflate(filename, CG_DIR))
    ...
    def _parse_json_cg(json_graph: load.JsonType) -> NodeMap:
        ...
        cg = {node['name']: CGNode(node, json_cfg[node['name']]) for node in json_cg.values()}
