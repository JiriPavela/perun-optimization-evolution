...
# TODO: needs update
def build_title(solution: sol.Solution) -> str:
    ...
    params_str = ''
    for name, value in solution.apply_parameters.items():
        params_str += rf'[${name} = {value:.3f}$] '
    # Identify the plot by the workload name and set its resulting fitness
    return rf"W={solution.workload.name}  [$fitness = {solution.fitness[0]:.3f}$] " + params_str
