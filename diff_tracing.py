...
if TYPE_CHECKING:
    from call_graph import CallGraph, CGNode, BasicBlock, NodeMap, CfInstr
...
class CallGraphDiff:
    ...
    def __init__(self, cg_A: CallGraph, cg_B: CallGraph) -> None:
        ...
        self.del_funcs: NodeMap = {
            name: cg_A.funcs[name] for name in cg_B.funcs.keys() - cg_A.funcs.keys()
        }
        # Name changes B -> A
    ...
    def _compare_cg(self, cg_A: CallGraph, cg_B: CallGraph) -> NodeMap:
        ...
        for func_B in funcs_to_inspect.values():
            ...
            if func_A.callees != callees_B or not self._same_cfg(func_A.cf, func_B.cf):
